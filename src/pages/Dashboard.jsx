...
export default function Dashboard({
  ...
  return (
    <motion.div
      className="max-w-4xl xl:max-w-6xl 2xl:max-w-7xl mx-auto flex flex-col gap-8"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.25 }}
    >
      {/* Intro section */}
      <section className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 pb-2">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="font-['Fraunces'] text-3xl sm:text-4xl lg:text-5xl font-semibold text-[var(--color-ink)] leading-tight">
            Let's make some<br /><em>progress.</em>
          </h1>
          <p className="text-sm text-[var(--color-ink-secondary)] leading-relaxed max-w-lg">{smartCopy.message}</p>
        </div>
        <div className="flex sm:flex-col items-center sm:items-center gap-3 sm:gap-1.5 shrink-0 p-3 sm:p-0 rounded-2xl bg-[var(--color-paper-card)] sm:bg-transparent border border-[var(--color-line-subtle)] sm:border-0">
          <Companion mood={smartCopy.mood} />
          <div className="flex flex-col sm:items-center">
            <span className="text-xs sm:text-[11px] font-semibold sm:font-normal text-[var(--color-ink)] sm:text-[var(--color-muted)] text-left sm:text-center whitespace-nowrap">
              {streak >= 2 ? `🔥 ${streak}-day streak` : 'Toki is cheering for you'}
            </span>
            <span className="text-[10px] text-[var(--color-muted)] sm:hidden">Keep going strong!</span>
          </div>
        </div>
      </section>

      {/* Progress bar with Plan My Day button */}
      <section className="flex flex-col sm:flex-row gap-4 p-5 rounded-2xl border border-[var(--color-line)] bg-[var(--color-paper-card)] shadow-[var(--shadow-sm)]" aria-label="Today productivity summary">
          ...
          <div className="flex items-center justify-between">
            ...
            {onOpenPlanMyDay && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--color-coral-subtle)] text-[var(--color-coral)] text-xs font-semibold hover:bg-[#f8dfd8] active:scale-95 transition-all"
              >
                <Sparkles size={13} />
        ...
        <div className="flex flex-col gap-1.5 sm:min-w-[200px] justify-center">
          ...
          <div className="h-2.5 rounded-full bg-[var(--color-paper-deep)] overflow-hidden" role="progressbar" aria-valuenow={todayStats.percent} aria-valuemin="0" aria-valuemax="100">
            <motion.i
      ...
      {/* Responsive 2-Column Grid on Large Screens / TVs */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        {/* Main Column (Tasks, QuickAdd, Focus) */}
        ...
        {/* Side Column on Large Screens (Insights & Activity) */}
